...
static bool parseDialectArgs(CompilerInvocation &res, llvm::opt::ArgList &args,
  ...
  // -fdefault* family
  if (const llvm::opt::Arg *arg =
          args.getLastArg(clang::driver::options::OPT_fdefault_real_8,
                          clang::driver::options::OPT_fdefault_real_4)) {
    const llvm::opt::Option &opt = arg->getOption();
    if (opt.matches(clang::driver::options::OPT_fdefault_real_8)) {
      res.getDefaultKinds().set_defaultRealKind(8);
      res.getDefaultKinds().set_doublePrecisionKind(16);
    } else if (opt.matches(clang::driver::options::OPT_fdefault_real_4)) {
      res.getDefaultKinds().set_defaultRealKind(4);
      res.getDefaultKinds().set_doublePrecisionKind(8);
    }
  }
  if (const llvm::opt::Arg *arg =
          args.getLastArg(clang::driver::options::OPT_fdefault_integer_8,
                          clang::driver::options::OPT_fdefault_integer_4)) {
    const llvm::opt::Option &opt = arg->getOption();
    if (opt.matches(clang::driver::options::OPT_fdefault_integer_8)) {
    ...
    } else if (opt.matches(clang::driver::options::OPT_fdefault_integer_4)) {
